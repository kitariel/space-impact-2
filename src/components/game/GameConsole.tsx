...
export function GameConsole({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={`game-console ${compact ? 'is-compact' : 'is-full'} display-${settings.displayMode}`}>
    ...
    <div className="lcd-bezel">
      <div className="lcd-screen" data-live-screen>
        ...
        {error ? <div className="game-menu"><h2>DISPLAY OFFLINE</h2><p>{error}</p></div> : menu && <GameMenu key={snapshot.status} compact={compact} snapshot={snapshot} best={settings.highScore} onStart={() => engine.current?.start()} onResume={() => engine.current?.resume()} onExit={() => engine.current?.attract()}/>}
      </div>
    </div>
    ...
    {active && <MobileControls ready={snapshot.special >= 1} onInput={(action, pressed) => engine.current?.input.setTouch(action, pressed)}/>}
